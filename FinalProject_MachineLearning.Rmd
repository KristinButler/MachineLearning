...
round(comb3, 2)
```
Comparing all four models, the random forrest was the most accurate one and was used for prediction on the test data. 




## Final model
The random forrest model was used for prection and below are the results in table from.  

```{r}
pre1 <- dplyr::select(pre, nam)
predict <- predict(mod1, testing2)
table(predict)
```
